...
exponent.
    ...
    def __init__(self, generator=2, group=17, keyLength=2048):
        ...
        if (keyLength < min_keyLength):
            print(
                "Error: keyLength is too small. Setting to minimum (", min_keyLength, ").")
...
"""
        ...
        if group in primes.keys():
            ...
            print("Error: No prime with group", group,
                  "Using default, ", default_group, ".")
    ...
    def displayParameters(self):
        ...
        print("Private Key[{0} bits]: {1}\n".format(
            self.privateKey.bit_length(), self.privateKey))
        print("Public Key[{0} bits]: {1}\n".format(
            self.publicKey.bit_length(), self.publicKey))
    ...
    def displayShared(self):
        ...
        print("Shared Secret[{0}]: {1}\n".format(
            self.sharedSecret.bit_length(), self.sharedSecret))
        print("Shared Key [{0}]: {1}\n".format(
            len(self.key), hexlify(self.key)))
